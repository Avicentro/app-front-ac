export const ROUTES = {
  INDEX: "/",
  NOT_FOUND: "*",
  LOGIN: "/login",
  TRAVEL: "/viaje",
  REST: "/descanso",
  USERS: "/usuarios",
  HISTORY: "/history",
  SUMMARY: "/resumen",
  PEOPLE: "/personas",
  SIGN_IN: "/sign-in",
  LOGBOOK: "/vitácora",
  PROGRAMMING: "/programacion",
  CREATE_USER: "/crear-usuario",
  CURRENT_TRAVEL: "/viaje-actual",
  ORDER_ENTRY: "/orden-de-entrada",
  CREATE_PEOPLE: "/crear-personas",
  ICE_PROVIDERS: "/proveedores-hielo",
  FORGOT_PASSWORD: "/olvido-su-contrasena",
  CHANGE_PASSWORD: "/cambio-de-contrasena",
};

export const COMPOSED_ROUTES = {
  REST: `${ROUTES.PROGRAMMING}${ROUTES.REST}`,
  TRAVEL: `${ROUTES.PROGRAMMING}${ROUTES.TRAVEL}`,
  CREATE_USER: `${ROUTES.USERS}${ROUTES.CREATE_USER}`,
  CREATE_PEOPLE: `${ROUTES.USERS}${ROUTES.CREATE_PEOPLE}`,
  SUMMARY_PROGRAMMING: `${ROUTES.PROGRAMMING}${ROUTES.SUMMARY}`,
  HISTORY_PROGRAMMING: `${ROUTES.PROGRAMMING}${ROUTES.HISTORY}`,
  LOGBOOK_PROGRAMMING: `${ROUTES.PROGRAMMING}${ROUTES.LOGBOOK}`,
  CURRENT_TRAVEL_PROGRAMMING: `${ROUTES.PROGRAMMING}${ROUTES.CURRENT_TRAVEL}`,
};
